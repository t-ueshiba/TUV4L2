/*
 * testV4L2camera: test program controlling an V4L2 1394-based Digital Camera
...
/*!
  \param userdata	MyV4L2CameraArray (V4L2カメラ配列)
*/
static void
CBup(GtkWidget* widget, gpointer userdata)
{
    static_cast<MyV4L2CameraArray*>(userdata)->up();
}
...
{
    static_cast<MyV4L2CameraArray*>(userdata)->down();
}
...
{
    static_cast<MyV4L2CameraArray*>(userdata)->view();
}

//! 選択されたファイルに設定内容をセーブするためのコールバック関数．
/*!
  \param userdataM	V4L2CameraArray (V4L2カメラ配列)
*/
static void
CBfileSelectionOK(GtkWidget* widget, gpointer userdata)
{
    const auto	cameras   = static_cast<MyV4L2CameraArray*>(userdata);
    if (cameras->size() > 0)
    {
...
{
    static_cast<MyV4L2CameraArray*>(userdata)->clear();
    gtk_main_quit();
}
...
/*!
  ...
  \param cameras	V4L2カメラの配列
  \return		生成されたコマンド群が貼りつけられたテーブル
*/
GtkWidget*
createCameraArrayCommands(MyV4L2CameraArray& cameras, GtkWidget* window)
{
